...
void convert_dates(
  ...
  if (conv == TO_YEAR) {
    /*@
      ...
      loop assigns remaining, val, results[0..entries-1], num_results, date_strings;

      loop variant remaining;
    */
    for (; date_strings != data_ptr_end; date_strings += TUPLE_WIDTH) {
      val ^= val;
      val += (date_strings[0] - 0x30) * 1000;
      val += (date_strings[1] - 0x30) * 100;
      val += (date_strings[2] - 0x30) * 10;
      val += (date_strings[3] - 0x30);
      results[num_results++] = val;
      //@ ghost remaining--;
    }
    ...
    /*@
      ...
      loop invariant is_digits(date_strings, remaining * TUPLE_WIDTH);
      loop invariant is_digits(date_strings+4, remaining * TUPLE_WIDTH-4);
    ...
    */
    ...
    for (; date_strings != data_ptr_end; date_strings += TUPLE_WIDTH) {
      val ^= val;
      val += (date_strings[4] - 0x30) * 10;
      val += (date_strings[5] - 0x30);
      //@ ghost uint32_t date_val = parse_string(date_strings+4, 2);
      //@ assert val == date_val;
      results[num_results++] = val;
      //@ ghost remaining--;
    }
    ...
    */
    for (; date_strings != data_ptr_end; date_strings += TUPLE_WIDTH) {
      val ^= val;
      val += (date_strings[6] - 0x30) * 10;
      val += (date_strings[7] - 0x30);
      //@ ghost uint32_t date_val = parse_string(date_strings+6, 2);
      //@ assert val == date_val;
      results[num_results++] = val;
      //@ ghost remaining--;
    }
