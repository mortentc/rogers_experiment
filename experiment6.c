...
int main(){
    ...
    mem_rng dummy;
    int element_id = allocate_shared(10, &dummy);
    ...
    // Load ebpf code
    FILE *fptr = fopen("programs/obtain_time.o", "rb");
    ...
    FILE *log = fopen("data/dependency.txt", "w");
    char out[10];
    uint64_t result;
    ...
    // Execute empty CSF
    // Warmup
    // request req_mock[1] = {{element_id, Write}};
    request csf_req[1] = {element_id, Read};
    request_set csf_release;
    csf_release.count = 1; csf_release.reqs = csf_req;
    for(int i = 0; i<WARMUP; i++) {
        obtain(imm, 1);
        ubpf_exec(vm, input, data_sz, &result, NULL, 0);
        enqueue_rel(&releases, immediate_release);
        enqueue_rel(&releases, csf_release);
    }
